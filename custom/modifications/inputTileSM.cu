...
#define k4d(i3, i2, i1, i0) Kc[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    X_shared[threadIdx.y][threadIdx.x]= x4d(n, c, h+threadIdx.y, w+threadIdx.x);
    __syncthreads();

      float acc = 0.;
      if(h < H_out && w < W_out && threadIdx.x < TILE_WIDTH && threadIdx.y < TILE_WIDTH){
        for(c=0; c<C; c++) {
          for(p=0; p<K; p++) {
            for(q=0; q<K; q++) {
              acc+= X_shared[threadIdx.y+p][threadIdx.x+q] * k4d(m, c, p, q);
...
__host__ void GPUInterface::conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    if (C == 1) {
      ...
      dim3 blockDim(TILE_WIDTH+6, TILE_WIDTH+6, 1);   //thread block size

  ...
  if (h < H_out && w < W_out) {
    for (int c = 0; c < C; c++) {
      ...
      for (d = 0; d < K*K - 1; d+=2) {
        ...
        p2 = (d+1) / 7;
        q2 = (d+1) % 7;
        k_half2 = __floats2half2_rn(kC4d(m,c,p1,q1), kC4d(m,c,p2,q2));
        x_half2 = __floats2half2_rn(X_shared[h0+p1][w0+q1], X_shared[h0+p2][w0+q2]);
        acc = __hadd2(acc, __hmul2(x_half2, k_half2));
      }
      k_half2 = __floats2half2_rn(kC4d(m,c,6,6), 0.0f);
      x_half2 = __floats2half2_rn(X_shared[h0+6][w0+6], 0.0f);
      acc = __hadd2(acc, __hmul2(x_half2, k_half2));
    ...
    y4d(n, m, h, w) = __half2float(total);
  }

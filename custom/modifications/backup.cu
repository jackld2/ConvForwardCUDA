...
#define kC4d(i3, i2, i1, i0) Kc[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    if (h < H_out && w < W_out) {
      for (c = 0; c < C; c++) {
        ...
        for (int i = h; i < h_base + X_tile_width; i++) {
          for (int j = w; j < w_base + X_tile_width; j++) {
            if (i < H && j < W){
...
#define kC4d(i3, i2, i1, i0) Kc[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    if (h < H_out && w < W_out) {
      for (c = 0; c < C; c++) {
        ...
        //   }
        if (h < H && w < W){
          X_shared[h - h_base][w - w_base] = x4d(n, c, h+3, w+3);
        }
        else {
          X_shared[h - h_base][w - w_base] = 0.0f;
        }
        __syncthreads();

        for (p = 0; p < K; p++) {
          for (q = 0; q < K; q++){
            // if (((h0+p-3) >= 0 && (w0+q-3) >= 0) && ((h0+p-3) < TILE_WIDTH && (w0+q-3) < TILE_WIDTH)) {
            //   acc += X_shared[h0+p-3][w0+q-3] * kC4d(m,c,p,q);
            // }
            // else {
            //   acc += x4d(n, c, h+p-3, w+q-3) * kC4d(m,c,p,q);
            // }
            float real = x4d(n, c, h+p, w+q)
            float theory =
            std::cout<<
            acc += real * kC4d(m,c,p,q);
...
#define kC4d(i3, i2, i1, i0) Kc[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    //     for (p = 0; p < K; p++) {
    //       for (q = 0; q < K; q++){
    //         if ((h0+p < TILE_WIDTH) && (w0+q < TILE_WIDTH)) {
    //           acc += X_shared[h0+p][w0+q] * kC4d(m,c,p,q);
    //         }
    //         else {
